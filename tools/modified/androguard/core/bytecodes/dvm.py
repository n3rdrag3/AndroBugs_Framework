...
from tools.modified.androguard.core.bytecodes.apk import APK
from tools.modified.androguard.core.androconf import CONF, debug, warning, is_android_raw
...
class EncodedMethod(object):
    ...
    def easy_print(self) :
        """
          Return the class name, method name, descriptor name of the method

          :rtype: string
        """
        return self.get_class_name() + "->" + self.get_name() + self.get_descriptor()
...
class Instruction(object):
    ...
    #Added by AndroBugs
    def get_kind_string(self):
      """
          Return the value of the formatted 'kind' argument
...
class ClassManager(object):
    ...
    #Added by AndroBugs
    def get_string_by_from_file_top_idx(self, idx_from_file) :
        try:
            if self.recode_ascii_string:
                if self.recode_ascii_string_meth:
                  return self.recode_ascii_string_meth(self.__strings_off[idx_from_file].get())
                return self.get_ascii_string(self.__strings_off[idx_from_file].get())
            return self.__strings_off[idx_from_file].get()
        except KeyError:
            bytecode.Warning( "unknown string item @ 0x%x(%d)" % (off,idx) )
            return "AG:IS: invalid string"

    #Added by AndroBugs
    def get_offset_idx_by_from_file_top_idx(self, idx) :
      try :
    ...
    def get_type_count(self) :
        return len(self.__manage_item[ "TYPE_TYPE_ID_ITEM" ])

	#Added by AndroBugs
    def get_field_count(self) :
        return len(self.__manage_item[ "TYPE_FIELD_ID_ITEM" ])

	#Added by AndroBugs
    def get_method_count(self) :
        return len(self.__manage_item[ "TYPE_METHOD_ID_ITEM" ])
...
class DalvikVMFormat(bytecode._Bytecode):
    ...
    #Added by AndroBugs
    def get_classes_in_dict(self):
        """
          Return a specific dictionary

          :rtype: a :dictionary: mapping `class` name string to `ClassDefItem` object
        """
        dic = {}

        for i in self.classes.class_def:
          if i.get_name() not in dic:
            dic[i.get_name()] = i

        return dic

    #Added by AndroBugs
    def get_specific_classidx_method_descriptor(self, class_idx, method_name, descriptor_name):
        for i in self.classes.class_def:
          if i.get_class_idx() == class_idx:
            for method in i.get_methods():
              if (method.get_name() == method_name) and (method.get_descriptor() == descriptor_name):
                return method
        return None

    #Added by AndroBugs
    def get_specific_class_method_descriptor(self, class_name, method_name, descriptor_name):
        for i in self.classes.class_def:
    ...
    def get_all_offset_from_file_and_string_value_mapping(self):
        ...
        return [i.get_off_and_value() for i in self.strings]
